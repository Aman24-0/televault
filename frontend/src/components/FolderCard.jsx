...
import { Folder, Edit2, Trash2, Move, CheckSquare, Square, ChevronRight } from 'lucide-react'

export default function FolderCard({ folder, onOpen, onRename, onDelete, onMove, index=0, renaming, renameVal, setRenameVal, doRename, cancelRename, isSelected, onToggleSelect }) {
  ...
  return (
    <motion.div
      ...
      className={`group relative bg-white/[0.02] border ${isSelected?'border-indigo-500/60 bg-indigo-500/5':'border-white/5 hover:border-white/10 hover:bg-white/[0.04]'} rounded-2xl p-4 transition-colors duration-150 cursor-pointer select-none`}
      onClick={() => renaming ? null : isSelected ? onToggleSelect() : onOpen()}
    >
      {/* Checkbox */}
