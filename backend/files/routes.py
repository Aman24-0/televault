...
# ── Advanced Secure Share Endpoint ───────────────────────────
...
async def toggle_share(file_id: str, body: ShareToggle, user=Depends(get_current_user)):
    ...
    async with pool.acquire() as db:
        ...
        if body.enabled:
            ...
            pwd = body.password if body.password else None
            
            await db.execute(
                "UPDATE files SET share_enabled=1, share_token=$1, share_password=$2, share_expires_at=$3 WHERE id=$4",
                token, pwd, exp, file_id
            )
            return {"share_token": token, "share_enabled": 1, "has_password": bool(pwd), "expires_at": exp}
        else:
            await db.execute(
                "UPDATE files SET share_enabled=0, share_password=NULL, share_expires_at=NULL WHERE id=$1", 
                file_id
            )
            return {"share_enabled": 0}
...
async def get_stats(user=Depends(get_current_user)):
    pool = await get_db()
    async with pool.acquire() as db:
        rows = await db.fetch("SELECT size_bytes, mime_type FROM files WHERE user_id=$1 AND upload_status='completed'", user["user_id"])
        
    stats = {"total": 0, "images": 0, "videos": 0, "audio": 0, "documents": 0, "others": 0}
    for row in rows:
        size = row["size_bytes"] or 0
        stats["total"] += size
        mime = (row["mime_type"] or "").lower()
        if mime.startswith("image/"): stats["images"] += size
        elif mime.startswith("video/"): stats["videos"] += size
        elif mime.startswith("audio/"): stats["audio"] += size
        elif mime.startswith("text/") or "pdf" in mime or "document" in mime or "msword" in mime or "excel" in mime or "csv" in mime:
