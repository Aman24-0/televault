"""
TeleVault v2 - Storage Routes (PostgreSQL version)
"""
...
async def upload(file: UploadFile = File(...), parent_id: str = Form("root"), user=Depends(get_current_user)):
    ...
    async with pool.acquire() as db:
        await db.execute(
            "INSERT INTO files (id,name,original_name,extension,mime_type,size_bytes,parent_id,user_id,upload_status) VALUES ($1,$2,$3,$4,$5,0,$6,$7,'pending')",
    ...
    try:
        ...
        if size > MAX_SIZE:
            async with pool.acquire() as db:
                await db.execute("DELETE FROM files WHERE id=$1", fid)
            raise HTTPException(413, "File too large. Maximum 500MB.")

        async with pool.acquire() as db:
            await db.execute("UPDATE files SET size_bytes=$1 WHERE id=$2", size, fid)
        ...
        async with pool.acquire() as db:
            row = await db.fetchrow(
                "UPDATE files SET upload_status='completed',telegram_message_id=$1,telegram_file_id=$2,thumbnail_msg_id=$3 WHERE id=$4 RETURNING *",
        ...
        async with pool.acquire() as db:
            await db.execute("UPDATE files SET upload_status='failed' WHERE id=$1", fid)
